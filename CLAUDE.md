...
- **Phase:** D (→ **G3 public launch Sat Sep 5**, slipped one day from Fri Sep 4). **SPEC v1 BUILD-COMPLETE 08-30**: metrics, transactions, playoffs+consolation+completion, belt/leaderboard/letter/roast, trades (clock-opens Sep 22), Tier 2 hosted (**signups OPEN 09-03**, pulled forward from G5). Done since G2: ToS, F2 audit, seed cards, BYOM copy, brand (favicon/hero/card mark), USPTO clear, banter loop + feed split. Engineering DONE (outbox `docs-private/OUTBOX.md` — local-only, gitignored — + `scripts/preflight.sh` shipped 08-30); `scripts/preflight.sh` is the gate. Post-review hardening shipped 08-30 (settlement completeness gate, caller-scoped idempotency, hosted economics, launch surface, kill-criteria metrics): **254 tests, redteam 105 CLEAN**. Workers Paid ✓ 09-01; OPENROUTER_ORG_KEY / HOSTED_AGENT_KEY_SECRET / OPERATOR_EMAIL set ✓. **09-01:** house fleet folded into the Worker runner (banter pacing now thread-derived: the 3-day round cap had left the front page silent for 22h+), advice gate extended to roster writes, Workers Logs on; **09-03:** fold soaked 24h clean (0 error lines, 90 banter msgs/day, ~$0.16/day) → `HOSTED_OPEN=1`, hosted signups live; the 09-01 500 storm closed (owner confirmed the Paid upgrade time = free-tier D1 cap); banter pacing retuned to 6h spacing × 4/day after the 3h × 3 rule left the fleet silent ~15h/day. **Repo public 09-03** (recreated from clean history, orphans gone, preflight GREEN). **09-04:** pre-launch audit found the 6h wire sync dead since 06Z Sep 3 (nflverse dropped `week` from rosters/); fixed with optional `week`, per-source isolation + alarm, and a preflight freshness gate. Human blockers: account grabs + Moltbook tweet (ADP review and launch screenshots closed 09-04). **09-05:** OpenRouter for Startups application PARKED — the form now requires a partner referral key (VC/accelerator/ecosystem partner); measured burn ~$0.15/day makes credits a nice-to-have, Nous pitch instead; `docs-private/OUTBOX.md` re-anchored to Sat Sep 5 (T-relative runbook, counters re-verified). Contact channel live 09-03: **commissioner@deepleague.app** (Cloudflare Email Routing → owner inbox; footer, ToS, `OPERATOR_EMAIL`, and Resend's from-address all use it). GTM: `docs-private/GTM.md` (local-only; D2/D3 post-G4).
- **09-08:** launch slipped to **Tue Sep 8** (nothing posted Sep 4–7; 0 external registrations; fleet/wire/settlement paths ran unattended clean). nflverse's 2026 injuries file dropped `report_primary_injury` — parser made tolerant, unknown players skipped, verified live at the 14:00Z sync. Kickoff is **Wed Sep 9 17:20 PT** per schedule data; every public league formed this week starts **Week 2** (`leagueStartWeek`), and the launch copy says so.
- **Next gates:** G3 Tue Sep 8 (slipped from Sep 4/5) · NFL Week 1 kickoff Wed Sep 9 17:20 PT · G4 first settlement Tue Sep 15 · G5 hosted tier Sep 18 (signups already open since 09-03; G5 = the review).
